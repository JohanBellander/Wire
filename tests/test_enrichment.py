...
def fake_client():
    ...
    c.compare_commits = AsyncMock(
        return_value=[
            {
                "sha": "abc",
                "commit": {"message": "feat: ship a thing", "author": {"name": "Johan"}},
            },
            {"sha": "def", "commit": {"message": "fix: bug", "author": {"name": "Johan"}}},
        ]
    )
    c.get_pull_request = AsyncMock(
        return_value={
            "id": 999,
            "number": 42,
...
async def test_enrich_pr_caches_within_one_call(fake_client):
    ...
    raw = [
        ...
        },
        {
            ...
            "payload": {"action": "closed", "number": 7, "pull_request": {"number": 7}},
        },
    ]
